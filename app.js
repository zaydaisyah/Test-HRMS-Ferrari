...
function initDatabase() {
  ...
  const requiredIds = ["IM018", "IM022", "IM030", "IM043", "IM005"];
...
function handleLogin(staffId) {
  ...
  const user = state.users[cleanedId];

  if (!user) {
    showToast("Invalid Staff ID. Example: IM022 (HR Admin) or IM043 (Staff).", "error");
    return false;
  }

  // Password check — required for admin/HR accounts
  if (user.isAdmin && user.password) {
    const pwdInput = document.getElementById("password-input");
    const enteredPwd = pwdInput ? pwdInput.value : "";
    if (!enteredPwd) {
      showToast("HR/Admin accounts require a password.", "error");
...
function renderLeaveHRApprove() {
  ...
  let requests = state.leaves.filter(l => l.status === "Pending" && l.staffId !== state.currentUser.id);
    ...
    pageRequests.forEach(l => {
      tbody.innerHTML += `
        <tr class="approve-leave-row" id="leave-row-${l.id}">
          ...
          <td><span class="badge pending">${l.status}</span></td>
          <td>${l.staffId}</td>
          <td style="font-weight: 600;">${l.staffName}</td>
...
safeAddListener("travel-hr-apply-form", "submit", function(e) {
  ...
  const newRequest = {
    ...
    costTotal: costTotal,
    advanceAmount: parseFloat(document.getElementById("travel-hr-advance").value) || 0
  };
...
safeAddListener("travel-staff-apply-form", "submit", function(e) {
  ...
  const newRequest = {
    ...
    costTotal: costTotal,
    advanceAmount: parseFloat(document.getElementById("travel-staff-advance").value) || 0
  };
...
function calculateFormTotals(isAdmin) {
  ...
  const costInputs = [
    lodgingTotal,
    document.getElementById(`${prefix}-mileage`),
  ...
  paper.innerHTML = `
    ...
    <div class="official-warrant-header">
      <div class="official-warrant-brand">
        ...
        <svg class="official-warrant-logo-svg" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
          <circle cx="30" cy="30" r="25" fill="#f8f9fc" stroke="#000000" stroke-width="2"/>
          <path d="M28 42c0-1.5 1.5-3.5 2-5h-3s1-3 1.5-4.5c.5-1.5.5-2-.5-2-1-.5-2 .5-3 1.5-.5-.5-.5-.8-.3-1.2.3-.5 1-1 2-1h3.5s-1 3.5-1.5 5c-.5 1.5-.5 2.5 0 2.5.5 0 1.5-1 2.5-2 .5.5.5.8.3 1.2-.5.5-1.5 1-2.5 1h-3s1.5-3.5 2-5" fill="#000000"/>
          <path d="M23 20c.5-1 1-3.5 2.5-4.5s4-1 4.5.5-.5 3-2.5 4-3.5 1.5-4.5 0z" fill="#000000"/>
          <path d="M27 25c1.5-2.5 3-3.5 4.5-3.5s2 1.5.5 4-3.5 3-4.5 3c-.2 0-.4-.4-.5-1z" fill="#d1121b"/>
        </svg>
    ...
    <table class="official-cost-table">
      ...
      <tr>
        ...
        <td class="cost-calc-col"><small>(if required kindly attach with ECARF)</small></td>
    ...
    <div class="official-warrant-header">
      <div class="official-warrant-brand">
        <svg class="official-warrant-logo-svg" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M28 42c0-1.5 1.5-3.5 2-5h-3s1-3 1.5-4.5c.5-1.5.5-2-.5-2-1-.5-2 .5-3 1.5-.5-.5-.5-.8-.3-1.2.3-.5 1-1 2-1h3.5s-1 3.5-1.5 5c-.5 1.5-.5 2.5 0 2.5.5 0 1.5-1 2.5-2 .5.5.5.8.3 1.2-.5.5-1.5 1-2.5 1h-3s1.5-3.5 2-5" fill="#000000"/>
          <path d="M23 20c.5-1 1-3.5 2.5-4.5s4-1 4.5.5-.5 3-2.5 4-3.5 1.5-4.5 0z" fill="#000000"/>
          <path d="M27 25c1.5-2.5 3-3.5 4.5-3.5s2 1.5.5 4-3.5 3-4.5 3c-.2 0-.4-.4-.5-1z" fill="#800000"/>
        </svg>
        <div class="official-warrant-title-text">
...
safeAddListener("leave-staff-apply-form", "submit", function(e) {
  ...
  this.reset();
  showToast("Leave request submitted successfully! Pending HR approval. Opening application PDF...");
  triggerConfetti();
...
safeAddListener("modal-confirm-btn", "click", function() {
  ...
  if (sysType === "travel") {
    ...
    if (actionType === "approve") {
      // Deduct days from staff balance
      const applicant = state.users[leave.staffId];
      const leaveTypeKey = leave.leaveType.toUpperCase();
  ...
  showToast(`Request ${reqId} has been successfully ${actionType === "approve" ? "approved" : "rejected"}.`);
...
function openProfileModal() {
  ...
  if (pwdContainer) {
    pwdContainer.style.display = isHR ? "block" : "none";
  }
